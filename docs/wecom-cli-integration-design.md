...
| 阶段 | 内容 | 人日 | 验收 |
| --- | --- | --- | --- |
| **P0** 通道可用 | 镜像内嵌 CLI 二进制 + 卷；`WeComCliClient`；`extra.botId` + `encrypted_bot_secret`（V83）；连接器校验/提示；`GET /api/wecom-cli/status`；`probeWeCom` 双通道探活 | 1–1.5 | 连接器页填 Bot 凭证后显示「机器人通道：已授权（Bot aib…）」；探活区分应用/机器人两通道 |
| **P1** 授权体验 | 扫码授权兜底（二维码弹窗 + 轮询）；**品类授权矩阵**（13 品类体检 + 状态徽标 + 续期链接透传） | 1–1.5 | 卡片能显示「待办=已过期 / 会议=未授权 / 消息=可用」并可点续期链接跳转企微 |
| **P2** AI 工具 | 首批 6 个工具（contact/todo/meeting/doc/message/mail）+ ID 不外露约束 | 1–2 | AI 助手可答「我的企微待办有哪些」「上周三的会议纪要在哪」 |
| **P3** 会议联动 | 企微会议纪要/转写导入 meeting 模块 + 总结流水线 | 1–2 | 一条企微会议 → 自动生成 meeting 记录（章节/关键词/待办） |
| **P4**（可选） | 定时巡检品类授权到期并告警；CLI 版本升级流程 | 0.5 | 授权到期前/后推送提醒到企微或站内 |

前置条件：**Bot 创建者需在企微侧为品类授权**（当前实测：消息/待办 已过期，会议/通讯录 未授权，文档 部分未授权）——这是 P2 出效果的关键，属**业务侧动作**，需提前协调。
