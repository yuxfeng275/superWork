...
>
> 存档说明（2026-09-25）：旧 Vue 前端已下线删除，文中「两套前端」及 `frontend/src/...` 路径仅作历史记录，当前唯一前端为 frontend-pro。
...
> **实施状态（2026-09-20）**：P0 / P1 / P2 已实现并上线 241；**P3 已放弃**（企微「会议」品类授权无法获取）；P4 未开始。
...
| **P1** 授权体验 | 扫码授权（二维码 + 5 分钟倒计时 + 3 秒轮询）；品类授权矩阵（7 品类体检、状态徽标、续期链接逐字透传）；两套前端连接器页「机器人通道」分区 | ✅ 已上线（实测矩阵：通讯录/文档/微盘/邮件=未授权，待办/日程=可用，消息=未对企开放，链接为企微真实续期地址） |
| **P2** AI 工具 | 9 个工具：通讯录搜索、待办列表/创建/完成、日程列表、文档搜索/读取、消息发送、邮件检索 | ✅ 已上线（真实数据实测：待办 读/创建/完成、日程 85 条、AI 对话产出带时间冲突提示的汇总） |
| **P3** 会议联动 | ~~企微会议纪要/转写导入 meeting 模块~~ | ❌ 已放弃：企微「会议」品类授权确认无法获取，相关工具/接口/数据源已移除（见 `docs/schedule-center-design.md` 第七节） |
| **P4**（可选） | 定时巡检品类授权到期并告警；CLI 版本升级流程 | ⏸ 未开始 |

**前置条件（业务侧动作）**：授权入口在连接器页品类矩阵的「点击这里」链接。注意两类不同路径（详见 4.2）：通讯录、待办、日程、消息等可由机器人创建者自助授权/续期；文档、微盘、邮件提示「需向管理员申请使用」，须走企微内申请 + 管理员审批；消息「发送」能力（853006）须企业管理员开通；**「会议」品类确认无法获取，已放弃接入**。
