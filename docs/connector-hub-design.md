...
| 维度 | 归属 | 内容 |
|---|---|---|
| 连接 | 连接器管理（`ai_connector`） | 外部系统（云效/工时/OA/语雀/邮件/企微）的地址、凭据、启停、连接测试、`extra_config` |
| 模型 | 模型管理（`ai_model`） | 提供方编码、接入协议、模型名、展示名、**接口地址 / API Key**、用途（助手 / 摘要 / 决策）、默认、启停 |

- 模型可自带 `base_url` + `encrypted_api_key`（官方、中转站、自建 OpenAI 兼容）；未填时才回落同名连接器。
- 可用性 = 模型「启用 + 勾选用途」且接入就绪（自带地址+Key，或同名连接器 READY）。
...
4. ~~**新旧前端并行**：两套前端都指向同一后端接口，改造必须同时落地，否则出现「旧页面调已删接口」。~~（历史约束：旧 Vue 前端已于 2026-09-25 下线，此后只需维护 frontend-pro）
