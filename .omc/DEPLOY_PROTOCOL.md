...
| 2026-09-18 | d265c18 | 用当前 master 重建 frontend-pro，把周进展弹窗标题（当周区间）与可拖拽进度条带回线上；此前 b57b7e7 日程中心部署覆盖了旧 dist。仅前端。备份 deploy-backups/20260918-140054，已重启 nginx 并核对 :18080=frontend-pro / umi.cb01074a.js 200 / p__key-matters__index.d8d619d9.async.js 200 / 旧 p__key-matters__index.d8d92ac5.async.js 404 | ui-fix agent |
| 2026-09-18 | 02cb2be | frontend-pro：周会左侧快速筛选，本周已更新大事儿名称前加绿色小点；项目/负责人分组汇总进度旁加小进度条。仅前端。备份 deploy-backups/20260918-140620，已重启 nginx 并核对 :18080=frontend-pro / umi.350b56e9.js 200 / p__key-matters__index.30d8cff6.async.js 200 / 旧 p__key-matters__index.d8d619d9.async.js 404 | ui-fix agent |
| 2026-09-18 | c866366 | frontend-pro：周会快速导航增加全部/已更新/未更新全局筛选（项目与负责人共用）；中间卡片展示截止日期；正文加大字号，空态改图标。仅前端。备份 deploy-backups/20260918-141953，已重启 nginx 并核对 :18080=frontend-pro / umi.23e5b77d.js 200 / p__key-matters__index.688055e6.async.js 200 / 旧 p__key-matters__index.30d8cff6.async.js 404 | ui-fix agent |
| 2026-09-18 | f49ed9a | frontend-pro：周会卡片空态去掉对号，统一改 Inbox 空盒子图标。仅前端。备份 deploy-backups/20260918-142321，已重启 nginx 并核对 :18080=frontend-pro / umi.4c66c17f.js 200 / p__key-matters__index.e20ce88d.async.js 200 / 旧 p__key-matters__index.688055e6.async.js 404 | ui-fix agent |
| 2026-09-18 | 466508d | 周进展/周会支持简易 Markdown 与 @姓名；被 @ 用户生成待办并进入消息铃铛；工作菜单新增待办页。backend + frontend-pro。备份 deploy-backups/20260918-164050，已重启 nginx 并核对 :18080=frontend-pro / umi.1bdf4d06.js 200 / p__todos__index.f1fcbd85.async.js 200 / p__key-matters__index.9fa36b71.async.js 200 / backend health 200 | ui-fix agent |
| 2026-09-18 | 0b5e6d5 | @姓名显示为蓝色链接，点击打开该员工待办列表。backend + frontend-pro。备份 deploy-backups/20260918-171130，已重启 nginx 并核对 :18080=frontend-pro / umi.65ba7260.js 200 / p__todos__index.b3a7adf4.async.js 200 / p__key-matters__index.489c8cef.async.js 200 / backend health 200 | ui-fix agent |
| 2026-09-19 | 0204c93 | 周报中心：精简 AI 生成口径；列表「同步」不再带开详情；汇总表回填改为写入语雀目标周次 K 列。backend + frontend-pro。备份 deploy-backups/20260919-221836，已重启 nginx 并核对 :18080=frontend-pro / umi.199374eb.js 200 / p__weekly-report__index.a06f59a0.async.js 200 / backend Started | ui-fix agent |
| 2026-09-20 | 160bf18 | 周报重新生成后立刻回填当前详情（ae28d01）；生成只提炼重点；汇总表回填 404 带文档路径并兼容表格文档。backend + frontend-pro，内网 192.168.1.241。备份 deploy-backups/20260920-093707，已重启 nginx 并核对 :18080=frontend-pro / umi.2f006687.js 200 / p__weekly-report__index.d9446f5a.async.js 200 / backend health 200 | ui-fix agent |
| 2026-09-20 | 2bfa0cd | 汇总表回填 404 改为明确提示：当前语雀 Token 无权访问 staff-qvc012/mghdgg。仅 backend。备份 deploy-backups/20260920-095938，backend health 200 | ui-fix agent |
| 2026-09-20 | 75a91c1 | 汇总表改为辅助人工粘贴：复制纪要链接、打开表格、确认后标记完成；不再用无权限 Token 强写。backend + frontend-pro。备份 deploy-backups/20260920-115132，已重启 nginx 并核对 :18080=frontend-pro / umi.e8df69ae.js 200 / p__weekly-report__index.50aaae72.async.js 200 / backend health 200 | ui-fix agent |
| 2026-09-20 | 6736ee7 | 已成交商机禁售前工时；周报纳入商机跟进；周报详情独立阅读页。backend + frontend-pro。备份 deploy-backups/20260920-171223，已重启 nginx 并核对 :18080=frontend-pro / umi.937e9ae6.js 200 / p__weekly-report__index.bced62c2.async.js 200 / p__opportunities__index.f2c2e451.async.js 200 / backend health 200 | ui-fix agent |
| 2026-09-21 | b056e49 | AI 助手按 Ant Design X 改会话列表、气泡、输入与欢迎区。仅 frontend-pro。备份 deploy-backups/20260921-092134，已重启 nginx 并核对 :18080=frontend-pro / umi.abbcfd3a.js 200 / p__ai-assistant__index.6d9cc977.async.js 200 | ui-fix agent |
| 2026-09-21 | 0064b11 | AI 助手补 ThoughtChain、/ 快捷指令 Suggestion、Sender 附件预览。仅 frontend-pro。备份 deploy-backups/20260921-092655，已重启 nginx 并核对 :18080=frontend-pro / umi.bea31c1a.js 200 / p__ai-assistant__index.5b7b169c.async.js 200 | ui-fix agent |
| 2026-09-21 | 9c9bd5f | AI 助手文本附件随问题发给后端，气泡支持复制。仅 frontend-pro。备份 deploy-backups/20260921-093158，已重启 nginx 并核对 :18080=frontend-pro / umi.927634ce.js 200 / p__ai-assistant__index.c0cde840.async.js 200 | ui-fix agent |
| 2026-09-21 | b4d5e59 | AI 助手会话附件本地落盘（V86 ai_agent_attachment + 上传/列表接口），历史存 [附件: x] 标记、侧车注入文本摘要；backend+frontend-pro。备份 deploy-backups/20260921-100501，backend --no-cache 重建、nginx 重启，健康 200 / V86 表已建 / umi.daf00807.js 200 / p__ai-assistant__index.a96d17f9.async.js 200 | ui-fix agent |
| 2026-09-22 | bf9fbe3 | ① TypeSafe Jev 接入（ffbb04e）：V87 typesafe 连接器 + 决策模型，意图路由收窄工具、写操作门禁（fail-open），对话 ThoughtChain 展示 Jev 判定；② 模型与连接器解耦（bf9fbe3）：V88 ai_model 增 api_protocol/base_url/encrypted_api_key，模型可自带 OpenAI 兼容地址（官方/中转站/自建），未填才回落同名连接器；侧车放开 OpenAI 兼容 provider（自定义必须带 baseUrl）；模型管理页手填提供方编码 + 协议/地址/Key，连接器页收口为外部系统职责。backend+frontend-pro+ai-sidecar。备份 deploy-backups/20260922-093754，内网 192.168.1.241，--no-cache 重建 backend/frontend-pro/ai-sidecar、nginx 重启，健康 200 / V87+V88 已迁移（typesafe 行 api_protocol=typesafe）/ umi.93c287bf.js 200 / p__ai-assistant__index.777dbac3.async.js 200 / sidecar healthz ok | ui-fix agent |
| 2026-09-22 | d59a815 | 模型管理增强 + 周报提示词（6f483fa）：① POST /api/ai/models/remote-models 拉取 OpenAI 兼容端点模型列表（中转站免手抄模型 ID）；② POST /api/ai/models/{id}/test 与 /test 在线测试（最小化对话请求 + 延迟返回）；③ 模型页重构：统计行/状态列/行内测试，弹窗按 基本信息/接入配置/用途与状态 分段 + AutoComplete 模型名；④ 周报 V89 增 generation_prompt/generation_provider，编辑器新增「生成指引」输入（生成时自动落库并拼入 AI 输入），列表/详情/编辑态展示生成模型提供方与接入，新增 GET /api/weekly-reports/generation-model。backend+frontend-pro。备份 deploy-backups/20260922-024828，内网 192.168.1.241，重建 backend/frontend-pro、nginx 重启，backend Started / V89 已迁移（generation_prompt+generation_provider 已建）/ 新端点 401 就绪 / umi.138d7852.js 200 / p__weekly-report__index.3481802a.async.js 200；注：BuKeyMatterLockIntegrationTest 为既有环境性失败（H2 user 关键字，与本次无关），打包时排除，其余 505 测试全绿 | ui-fix agent |
| 2026-09-22 | f962833 | 模型管理去连接器化（eda9170）：列表页头部移除「连接器管理」入口、未就绪 Alert 改纯本页引导、表格/弹窗文案去连接器化；typesafe 决策模型行内「测试」禁用（不走 OpenAI 兼容协议，点击原会报 400）并加说明，弹窗测试前置拦截。仅 frontend-pro。备份 deploy-backups/20260922-035709，内网 192.168.1.241，重建 frontend-pro、nginx 重启，umi.b97c56f5.js 200（新文案在、「去连接器管理」已移除）。另查明：bai 模型测试失败根因是 241 出口被墙（api.b.ai DNS 投毒 + SNI 重置），非服务问题 | ui-fix agent |
| 2026-09-23 | dee52df | **按月待交付统计与销售确认**（f6c5449+修复）：V90 增 revenue_contract_entry.sales_owner（承接人缺省报价人，工时同步/OA/Excel 三链路写入）+ revenue_delivery_confirmation 表（月份×业务线×销售 唯一，PENDING/CONFIRMABLE/UNCONFIRMABLE+备注+确认人）；API /api/revenue/pending-delivery/{stats,entries,confirm}；frontend-pro 营收页新增「待交付确认」页签（/revenue/delivery-confirm，统计卡+确认弹窗+明细抽屉）。事故与修复：V90 初版 year_month 未加反引号撞 MySQL 保留字致 flyway 失败后端 crash-loop，已改反引号+ALTER 幂等，手工建表+删 flyway 失败行后重启恢复。存量 178 行 sales_owner 经工时 API 回填（178/212）。509 后端测试绿（BuKeyMatterLockIntegrationTest 为既有 H2 环境问题排除）。备份 deploy-backups/20260923-014300，内网 192.168.1.241，重建 backend/frontend-pro、nginx 重启，backend Started / V90 success=1 / stats 端点 401 / umi.5b9f99a1.js 200 / p__revenue__index.5b0e7c5a.async.js 200 | ui-fix agent |
| 2026-09-23 | cf01924 | **bai 模型经 mihomo 代理接通**（f78adf7+cf01924）：AiModelConfigService.ModelConfig 增 proxy（同名连接器 extra.proxy），模型测试/拉取列表/AI 助手侧车全链路走 HTTP CONNECT 前代；ai-sidecar 增 undici ProxyAgent 包装 streamFn fetch（RunRequest.proxy）；测试 max_tokens 1→4 兼容 b.ai 下限。生产操作：新建 bai 连接器（extra.proxy=http://mihomo:7890）、bai 模型启用、模型名大小写修正为 deepseek-v4.1-flash（按 remote-models 实拉清单）。实测：bai 测试 200 对话正常 1683ms、remote-models 拉到 25+ 模型、deepseek 直连回归 800ms 正常。backend+ai-sidecar。备份 deploy-backups/20260923-022535，重建 backend/ai-sidecar、nginx 重启，backend Started / 525 测试绿 | ui-fix agent |
| 2026-09-23 | 80beeea | 待交付确认页 400 修复：revenue_delivery_confirmation 实体 year_month 加 @TableField 反引号（MyBatis-Plus 生成 SQL 撞 MySQL 保留字，同 V34 既有惯例），并补回编辑中误删的 salesOwner 字段。实测：stats 200 返回真实聚合（16 组：周雪莹/高莹/陈柳等）、confirm 写入→stats 联动→重置 PENDING 全链路通过。仅 backend。沿用备份 deploy-backups/20260923-022535，重建 backend | ui-fix agent |
| 2026-09-23 | eb6a1c4 | **营收子页独立化**（8fcf0ec）：营收页去掉 Tab 条，路由即页面——/revenue/worktime 工时&成本（含营收估算卡）、/revenue/delivery 交付与利润、/revenue/delivery-confirm 待交付确认、/revenue/import 数据导入（含交付合同管理）、/revenue/pending 待映射与销售项目，每页独立标题/说明，统计卡仅在工时/交付页显示；V92 新增「待交付确认」子菜单（数据组下，授权复制自 工时&成本，5 角色）。backend+frontend-pro。备份 deploy-backups/20260923-060705，重建 backend/frontend-pro、nginx 重启，backend Started / V92 success=1 / 菜单行+授权就位 / umi.240a9025.js 200 / p__revenue__index.39627d42.async.js 200 | ui-fix agent |
| 2026-09-23 | 3b40e57 | **待交付口径修正 + 剔除测试合同**（be6ecfb）：待交付月份维度由收款销售月份改为**应收日期月**（空回落销售月）——修正前 Q4 待交付完全缺失（会员通 11 月 7 万、云鹿定制 12 月 80.6 万等）；V93 增 receivable_date（三链路摄入+upsert），存量 176 行经工时 API 回填；「测试」开头的测试合同三链路摄入跳过+统计/明细兜底剔除，prod 删除 3 条测试合同（cs）。待交付确认页年份扩到 去年/今年/明年。backend+frontend-pro。备份 deploy-backups/20260923-064236，重建 backend/frontend-pro、nginx 重启，V93 success=1 / stats 实测 7 组待交付含 Q4 / 527 测试绿 | ui-fix agent |
| 2026-09-23 | e150396 | 待交付确认表格单元格合并：同月份、同 月份+业务线 的连续行做纵向 rowSpan 合并（后端排序保证连续）。仅 frontend-pro。沿用备份 deploy-backups/20260923-064236，重建 frontend-pro、nginx 重启，p__revenue__index.6537d137.async.js 200 | ui-fix agent |
| 2026-09-23 | 38d6933 | **待交付口径按业务定义定稿**（38d6933）：待交付 = 交付日期为空 且 服务结束时间在对应月（月份维度 服务结束月→应收月→销售月回落；已填交付日期一律不计入）。V94 增 service_end_date（收款-项目服务完成日期，三链路摄入+upsert），存量 175 行回填。实测与工时源数据逐笔一致：10月 云鹿定制12.8万+会员通7万，11月 云鹿定制9.3万+会员通3.7万，12月 云鹿定制38.6万+会员通12.3万。backend+frontend-pro。备份 deploy-backups/20260923-083842，重建 backend/frontend-pro、nginx 重启，V94 success=1 / 527 测试绿 | ui-fix agent |
| 2026-09-22 | ba0d925 | **会议转写术语纠偏上线（LLM 提案 + Jev 决策门控）**：流水线新增纠偏阶段（转写后/总结前；术语表复用 meeting.asr.hotwords，空则跳过）；LLM 只列改动段（越界 seq/空/相同/长度暴增忽略），改动段 edited=true 落 corrected_transcript_json，摘要/待办自动用纠偏稿；Jev 两道 Noul 守门（是否需要纠偏/提案可采纳），决策模型未配置时降级无门控；生产验收（双人样本）：c d p→CDP、买点→埋点、马测→MA、皮看板→BI看板，关键词 [CDP,MA,BI,埋点,看板]，待办术语正确。顺带修复：H2 测试库 NON_KEYWORDS=USER + BuKeyMatterLockIntegrationTest 补 user/system_config_item/user_todo 表（该测试在 master 上既有失败，已修绿，全量 verify 通过）。备份 deploy-backups/superwork-meeting-correction-20260922-223227 | voice agent |
| 2026-09-23 | 2c90eb9 | **mihomo 出口代理上线 + Jev 纠偏门控生产实证**：241 新增 superwork-bu-mihomo（订阅节点，本地二进制镜像绕开 Docker Hub 限速；仅 bu-network 内网，默认直连，typesafe.ai/b.ai 走节点）；后端连接器 extra.proxy 支持 CONNECT 前代（TLS 端到端）；JevClient 端点归一化（模型行可写全端点 /v1/systemone，不再重复拼接）+ proxy 重载 + connectorProxy()；纠偏两道 Noul 门控生产实证：门控①需要纠偏→是（noul=0.95）、门控②提案可采纳→是（noul=0.6），model=jev-1.13.0 **直连成功**（当前无需代理，被墙时给 typesafe 连接器配 extra.proxy=http://mihomo:7890 即可切换）。备份 deploy-backups/superwork-proxy-20260923-005534 | voice agent |
| 2026-09-23 | f933abe | **「会议与日程」合并菜单上线**：V91 新增顶级菜单「会议与日程」（/meeting-schedule 伪路径分组，sort=2，Calendar 图标），会议(/meetings)、日程(/schedule) 降为其二级子项（sort 1/2，子项授权与权限 menu_id 不变）；父菜单授权五管理角色；菜单树验证：会议与日程 → 会议/日程 二级渲染正常。注意：并行 agent 的 V90（pending_delivery_confirm）首次应用失败（瞬时，重试自愈，V90/V91 均 success=1）。备份 deploy-backups/superwork-v91-20260923-094914 | voice agent |
...
| 2026-09-18 | b57b7e7 | **日程中心上线（会议 × 企微日程合并）**：新增「日程」页（V84 菜单 + `schedule:view` 授权五管理角色），后端 `/api/schedule/events` 聚合**本地会议**（会议模块记录，按上传人可见）与**企微日程**（wecom-cli 日历），并按企微 `repeat_rule` 展开周期场次（daily/weekly/work_day/monthly/monthly_on_the_nth_day/yearly/yearly_on_the_nth_day + interval/次数/repeat_until/exception）；企微窗口按当天 ±29 天钳制（避免 90747），企微侧失败只出 hint 不阻断本地数据。前端两套均提供**日历视图**（月网格 + 来源分色 chip + 当日面板）与**列表模式**（按天分组时间轴）；会议管理页加「日历视图」入口；旧 Vue 侧边栏支持「已注册路由的单页一级菜单」。实测真实数据：2026-09 日历 120 条 / 列表 108 条（本地会议 4 + 企微日程展开 104），无时区偏移 | system-config agent |
