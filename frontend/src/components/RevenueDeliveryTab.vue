...
interface RowContext {
  ...
  /** true=含销售合计行：工时列=工时+销售工时、成本列=工时成本+销售成本（销售列不再单列） */
  combinedSales?: boolean
  lineSpan: number
...
/** 全表附加行：工时=工时+销售工时、成本=工时成本+销售成本，与「工时&成本」页全表口径对齐（均不含业务线级项目工时） */
const grandCombinedRow = (grand: RowContext): RowContext => {
  const periods = {} as Record<PeriodKey, PeriodView>
  ;(['h1', 'h2', 'ytd'] as PeriodKey[]).forEach(key => {
    const view = grand.periods[key]
    periods[key] = {
      ...view,
      projectHours: view.projectHours + view.salesHours,
      projectLaborCost: view.projectLaborCost + view.salesCost,
      salesHours: 0,
      salesCost: 0,
      allocatedSalesHours: 0,
      allocatedSalesCost: 0,
      unallocatedSalesHours: 0,
      unallocatedSalesCost: 0
    }
  })
  return {
    ...grand,
...
const flatRows = computed<RowContext[]>(() => {
  ...
  const grand = grandTotalRow(data)
  rows.push(grand)
  rows.push(grandCombinedRow(grand))
  return rows
})
        ...
        <p class="overview-note">
          <el-popover placement="bottom-start" :width="400" trigger="hover" effect="dark">
            ...
            <div class="caliber-help-body">
              ...
              <p>末行「全表（含销售）」：工时列=工时+销售工时、成本列=工时成本+销售成本，与「工时&成本」页全表口径对齐（两边均不含业务线级项目工时）。</p>
              ...
              <template v-for="row in flatRows" :key="rowKey(row)">
                <tr :class="row.kind === 'line' ? 'line-total-row' : row.kind === 'grand' ? 'grand-total-row' : ''">
                  ...
                  <template v-for="column in periodColumns" :key="`c-${rowKey(row)}-${selectedPeriod}-${column.key}`">
                    <td
                      class="cell"
                      :class="[column.key === 'profit' && !row.combinedSales ? 'cell-profit clickable' : '', cellTone(row.periods[selectedPeriod], column.key)]"
                      :title="column.key === 'profit' && !row.combinedSales ? '点击查看利润构成' : undefined"
                      @click="column.key === 'profit' && !row.combinedSales && openProfitDetail(row, selectedPeriod)"
                    >
                      <el-tooltip
