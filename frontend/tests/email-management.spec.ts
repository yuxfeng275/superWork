...
test('管理角色可在页面加密保存 DeepSeek 与企业微信配置且密钥不回显', async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem('user', JSON.stringify({
      id: 10,
      username: 'admin',
  ...
  const status = {
    ...
    weComSecretConfigured: true,
    publicBaseUrl: 'http://192.168.1.241:18080'
  }
  await page.route('**/api/emails/integration-config', async route => {
    if (route.request().method() === 'PUT') {
      submitted = route.request().postDataJSON() as Record<string, unknown>
    }
    return fulfill(route, status)
  })
  await page.route('**/api/emails/account', route => fulfill(route, {
    configured: false,
    enabled: false,
    provider: 'ALIBABA_CLOUD_ENTERPRISE_MAIL',
  ...
  await page.goto('/emails')
  await expect(page.getByRole('heading', { name: '摘要与推送配置' })).toBeVisible()
  await page.getByLabel('DeepSeek API Key').fill('deepseek-page-secret')
  await page.getByLabel('企业微信 Secret').fill('wecom-page-secret')
  await page.getByRole('button', { name: '保存集成配置' }).click()

  await expect.poll(() => submitted).toMatchObject({
    deepSeekEnabled: true,
    deepSeekApiKey: 'deepseek-page-secret',
    weComEnabled: true,
    weComSecret: 'wecom-page-secret'
  })
  ...
  await expect(page.getByText('Secret 已配置')).toBeVisible()
})
